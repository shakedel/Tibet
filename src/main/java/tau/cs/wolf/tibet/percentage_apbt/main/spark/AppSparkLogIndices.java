...
import tau.cs.wolf.tibet.percentage_apbt.main.spark.functions.LogIndices;
...
public class AppSparkLogIndices extends AppBase {
	...
	public AppSparkLogIndices(ArgsSparkCommon args, JavaSparkContext ctx) throws IOException, ClassNotFoundException {
		super(args, null);
		this.ctx = ctx;
		this.args = args;
	...
	public static void main(String[] args) throws IOException, CmdLineException, ClassNotFoundException {
		try (JavaSparkContext ctx = new JavaSparkContext(new SparkConf())) {
			new AppSparkLogIndices(new ArgsSparkCommon(args), ctx).run();
		}
	}
