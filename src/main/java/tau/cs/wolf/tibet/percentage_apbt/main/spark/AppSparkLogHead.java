...
public class AppSparkLogHead extends AppBase {
	...
	public void run() {
		...
		LogHead logHead = new LogHead(this.bcastArgs, this.bcastProps, this.bcastClientProps, this.bcastEntries);
		if (this.args.isAsynced()) {
			filteredIndexPairs.foreachPartitionAsync(logHead);
		} else {
			filteredIndexPairs.foreachPartition(logHead);
		}
