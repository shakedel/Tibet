...
public class Union extends BaseModule {
	...
	public List<MatchResult> uniteMatches(List<MatchResult> matches) {
		...
		for (MatchResult unitedMatch: unitedMatches) {
			int tScore = -1*(unitedMatch.workInterval.getEnd().getIndex1() - unitedMatch.workInterval.getStart().getIndex1());
	...
	private boolean checkUnion(MatchPair pair) {
		if (!checkSegment(pair.left.workInterval.getSpan1(),
					      pair.right.workInterval.getSpan1())) {
			return false;
		}
		if (!checkSegment(pair.left.workInterval.getSpan2(),
						  pair.right.workInterval.getSpan2())) {
			return false;
		}
		return true;
	}
	...
	private MatchResult uniteMatches(MatchResult m1, MatchResult m2) {
		...
		return new MatchResult(Interval.newIntervalBySpans(unitedSpan1, unitedSpan2), -1, -1, -1);
	}
