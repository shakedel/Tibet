...
public class BYGUmapString
{
	 ...
	 */	
		public BYGUmapString(SuffixTreeNode tree1, SuffixTreeNode tree2,
		...
		{			
			...
			int[][] cornerVals=(int[][])_previousResults.get(prevI+"_"+prevJ);
			int[][] leftVals=(int[][])_previousResults.get(currI+"_"+prevJ);
			int[][] upperVals=(int[][])_previousResults.get(prevI+"_"+currJ);
			
			if(_monitor.get(prevI+"_"+prevJ)!=null)
			{
				...
				if(prevlastRow>=_table.length-1 || prevlastCol>=_table[prevlastRow].length-1)
					return;
				...
				_previousResults.put(curr1.orderNumber+"_"+curr2.orderNumber,tableToSave);
				if(continuecompare)
					_monitor.put(curr1.orderNumber+"_"+curr2.orderNumber,new Boolean(continuecompare));
			}
		...
		private void fillTableForChild1Root2(SuffixTreeNode curr1,SuffixTreeNode root2)
		{
			...
			int[][] prevValues=(int[][])_previousResults.get(prevI+"_"+prevJ);
			if(_monitor.get(prevI+"_"+prevJ)!=null)
			{
			...
			int [][] tableToSave=new int[4][];
				...
				_previousResults.put(curr1.orderNumber+"_"+0,tableToSave);
				if(continuecompare)
					_monitor.put(curr1.orderNumber+"_"+0,new Boolean(continuecompare));
			}
		}
		...
		private void fillTableForRoot1Children2(SuffixTreeNode root1, List children2)
		{
			for(int c=0;c<children2.size();c++)
			{
				...
				if(_monitor.get(prevI+"_"+prevJ)!=null)
				{
					...
					_previousResults.put(0+"_"+child2.orderNumber,tableToSave);
					if(continuecompare)
						_monitor.put(0+"_"+child2.orderNumber,new Boolean(continuecompare));
				}
		...
		private void fillTableForRoots(SuffixTreeNode root1,SuffixTreeNode root2)
		{
			...
			_previousResults.put(0+"_"+0,tableToSave);
			if(continuecompare)
				_monitor.put(0+"_"+0,new Boolean(continuecompare));
		...
		public static void main(String [] args)
		{
			...
			BYGUmapString algorithm=new BYGUmapString(tree1,tree2,
