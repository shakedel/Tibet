...
public class BYGU1 
{
	 ...
	 */	
		public BYGU1(SuffixTreeNode tree1, SuffixTreeNode tree2,
				String seq1, String terminator1, String seq2, String terminator2,int maxDiff, int minLen)
		{
			...
			_minLength=minLen;
			_maxDiff=maxDiff+1;
			_maxLength=Utils.min(_minLength, Utils.max(_length1, _length2));
			INFINITY=_maxDiff+1;	
		...
		private void processSubtrees(SuffixTreeNode root1, SuffixTreeNode root2)
		{
			...
			_previousResults=new Object[root1.totalChildren+1][root2.totalChildren+1];
			_monitor=new boolean[root1.totalChildren+1][root2.totalChildren+1];
		...
		private void fillTableForChild1ChildrenRoot2(SuffixTreeNode curr1, SuffixTreeNode subtree2)
		{
			...
			if(subtree2children!=null&& subtree2.children.size()>0)
			{
				for(int i=0;i<subtree2.children.size();i++)
				{
					...
					if(i==subtree2.children.size()-1)
					{
						_previousResults[curr1.parentOrderNumber][curr2.parentOrderNumber]=null;
		...
		{			
			...
			try
			{
			if(prevI<_previousResults.length && prevJ<_previousResults[currI].length)
			{
			...
			if(_monitor[prevI][prevJ])
			{
				...
				lastCol=Utils.min(prevlastCol+1+curr2.labelEnd-curr2.labelStart,_table[0].length-1);
			...
			if(curr2children!=null && curr2children.size()>0 )
			{
				for(int i=0;i<curr2children.size();i++)
				{
					...
					if(i==curr2children.size()-1)
					{
						_previousResults[curr1.parentOrderNumber][curr2child.parentOrderNumber]=null;					
					}
				}
			}
		}
		...
		private void fillTableForChild1Root2(SuffixTreeNode curr1,SuffixTreeNode root2)
		{
			...
			int prevJ=0;
			int prevI=curr1.parentOrderNumber;
			if(prevI<_previousResults.length && prevJ<_previousResults[prevI].length)
			{
		...
		public static void main(String [] args)
		{
			...
			BYGU1 algorithm=new BYGU1(tree1,tree2,
