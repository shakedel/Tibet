...
public class BYGUmap
{
	 ...
	 */	
		public BYGUmap(SuffixTreeNode tree1, SuffixTreeNode tree2,
		...
		private void processSubtrees(SuffixTreeNode root1, SuffixTreeNode root2)
		{
			...
			_previousResults=new HashMap();
			_monitor=new HashMap();
			totalchildren1=root1.totalChildren+1+root2.totalChildren+1;
			System.out.println("Now processing root1 with "+totalchildren1+" children via root2 with "+root2.totalChildren+" children.");
		...
		{			
			...
			int[][] cornerVals=(int[][])_previousResults.get(new Long(prevI*totalchildren1+prevJ));
			int[][] leftVals=(int[][])_previousResults.get(new Long(currI*totalchildren1+prevJ));
			int[][] upperVals=(int[][])_previousResults.get(new Long(prevI*totalchildren1+currJ));
			
			if(_monitor.get(new Long(prevI*totalchildren1+prevJ))!=null)
			{
				...
				if(prevlastRow>=_table.length-2 || prevlastCol>=_table[prevlastRow].length-2)
					return;
				...
				_previousResults.put(new Long(curr1.orderNumber*totalchildren1+curr2.orderNumber),tableToSave);
				if(continuecompare)
					_monitor.put(new Long(curr1.orderNumber*totalchildren1+curr2.orderNumber),new Boolean(continuecompare));
			}
		...
		private void fillTableForChild1Root2(SuffixTreeNode curr1,SuffixTreeNode root2)
		{
			...
			int[][] prevValues=(int[][])_previousResults.get(new Long(prevI*totalchildren1+prevJ));
			if(_monitor.get(new Long(prevI*totalchildren1+prevJ))!=null)
			{
			...
			int [][] tableToSave=new int[4][];
				...
				_previousResults.put(new Long(curr1.orderNumber*totalchildren1+0),tableToSave);
				if(continuecompare)
					_monitor.put(new Long(curr1.orderNumber*totalchildren1+0),new Boolean(continuecompare));
			}
		}
		...
		private void fillTableForRoot1Children2(SuffixTreeNode root1, List children2)
		{
			for(int c=0;c<children2.size();c++)
			{
				...
				if(_monitor.get(new Long(prevI*totalchildren1+prevJ))!=null)
				{
					...
					_previousResults.put(new Long(0*totalchildren1+child2.orderNumber),tableToSave);
					if(continuecompare)
						_monitor.put(new Long(0*totalchildren1+child2.orderNumber),new Boolean(continuecompare));
				}
		...
		private void appendChildLeaves(List res, SuffixTreeNode parent)
		{
			...
			{
				res.add(new Long(parent.leafNumber));
				return;
			}
		...
		private void fillTableForRoots(SuffixTreeNode root1,SuffixTreeNode root2)
		{
			...
			_previousResults.put(new Long(0),tableToSave);
			if(continuecompare)
				_monitor.put(new Long(0),new Boolean(continuecompare));
		...
		public static void main(String [] args)
		{
			...
			BYGUmap algorithm=new BYGUmap(tree1,tree2,
