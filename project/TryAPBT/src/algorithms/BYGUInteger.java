...
public class BYGUInteger 
{
	 ...
	 */	
		public BYGUInteger(SuffixTreeNode tree1, SuffixTreeNode tree2,
		...
		{			
			...
			Integer[][] cornerVals=(Integer[][])_previousResults[prevI][prevJ];
			Integer[][] leftVals=(Integer[][])_previousResults[currI][prevJ];
			Integer[][] upperVals=(Integer[][])_previousResults[prevI][currJ];
			
			if(_monitor[prevI][prevJ])
			{
				_table=new Integer [_maxLength+2][_maxLength+2];
				...
				Integer[] prevrowheader=leftVals[ROW_HEADER];
				Integer[] prevcolheader=upperVals[COL_HEADER];
				Integer[] prevlastRowVals=upperVals[LAST_ROW];
				Integer[] prevlastColVals=leftVals[LAST_COL];
				...
				for(int i=prevlastRow+1,k=prevlastRow+1;i<=lastRow;i++,k++)
				{
					...
					for(int j=Utils.max(prevlastCol+1,k-_maxDiff);j<=Utils.min(lastCol,k+_maxDiff);j++)
					{
						...
						_table[i][j]=new Integer
						(Utils.min
								(
								_table[i-1][j-1].intValue()+
								edScore(_seq1[_table[i][0].intValue()],
										_seq2[_table[0][j].intValue()]),
								infinity(_table[i][j-1])+1,
								infinity(_table[i-1][j])+1
								)
						);	
						
						if(_produceOutput && i-1>=_minLength && j-1>=_minLength && _table[i][j].intValue()<=_maxDiff 
								&& _seq1[_table[i][0].intValue()]!=_terminator1 && _seq2[_table[0][j].intValue()]!=_terminator2)
				...
				Integer [] lastColVals=new Integer[lastRow-1];
				Integer[] upperprevlastColVals=upperVals[LAST_COL];
		...
		private void fillTableForChild1Root2(SuffixTreeNode curr1,SuffixTreeNode root2)
		{
			...
			Integer[][] prevValues=(Integer[][])_previousResults[prevI][prevJ];
			if(_monitor[prevI][prevJ])
			{
				
				Integer[] prevrowheader=prevValues[ROW_HEADER];
				Integer[] prevcolheader=prevValues[COL_HEADER];
				Integer[] prevlastRowVals=prevValues[LAST_ROW];
				Integer[] prevlastColVals=prevValues[LAST_COL];
				...
				_table=new Integer [Utils.min(_maxLength+2,lastRow+1)][Utils.min(_maxLength+2,lastCol+1)];
		...
		private void fillTableForRoots(SuffixTreeNode root1,SuffixTreeNode root2)
		{
			...
			for(int i=2,k=2;i<=lastRow;i++,k++)
			{
				for(int j=Utils.max(2,k-_maxDiff);j<=Utils.min(lastCol,k+_maxDiff);j++)
				{
					...
					(Utils.min
							(
							_table[i-1][j-1].intValue()+edScore(_seq1[_table[i][0].intValue()],_seq2[_table[0][j].intValue()]),
							infinity(_table[i][j-1])+1,
							infinity(_table[i-1][j])+1
							)
					);
					if(i-1>=_minLength && j-1>=_minLength && _table[i][j].intValue()<=_maxDiff
							&& _seq1[_table[i][0].intValue()]!=_terminator1 && _seq2[_table[0][j].intValue()]!=_terminator2)
						addToSolution(allLeaves1,allLeaves2,i-1,j-1);
				}
			}
			...
			for(int i=0;i<positions1.size();i++)
			{
				...
				for(int j=0;j<positions2.size();j++)
				{
					...
					if((freeEnds && _seq1[startI]==_seq2[startJ]&& _seq1[endI]==_seq2[endJ]
					        && _table[len1][len2].intValue()<=Utils.min(infinity(_table[len1+1][len2]),infinity(_table[len1][len2+1]))+1)
					        ||!freeEnds)
					{
		...
		public static void main(String [] args)
		{
			...
			BYGUInteger algorithm=new BYGUInteger(tree1,tree2,
