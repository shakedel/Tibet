...
public class BYGU1 {
   ...
   public BYGU1(SuffixTreeNode tree1, SuffixTreeNode tree2, String seq1, String terminator1, String seq2, String terminator2, int maxDiff, int minLen) {
      this._tree1 = tree1;
      this._tree2 = tree2;
      ...
      this._maxLength = general.Utils.min(this._minLength, general.Utils.max(this._length1, this._length2));
   ...
   private void processSubtrees(SuffixTreeNode root1, SuffixTreeNode root2) {
      if(!this.freeEnds || this._seq1[root1.labelStart] == this._seq2[root2.labelStart]) {
         System.out.println("Comparing root1 with " + root1.totalChildren + " via root2 with " + root2.totalChildren);
         this._previousResults = new Object[root1.totalChildren + 1][root2.totalChildren + 1];
         this._monitor = new boolean[root1.totalChildren + 1][root2.totalChildren + 1];
   ...
   private void fillTableForChild1ChildrenRoot2(SuffixTreeNode curr1, SuffixTreeNode subtree2) {
      ...
      if(subtree2children != null && subtree2.children.size() > 0) {
         for(int curr1children = 0; curr1children < subtree2.children.size(); ++curr1children) {
            ...
            if(curr1children == subtree2.children.size() - 1) {
               this._previousResults[curr1.parentOrderNumber][i.parentOrderNumber] = null;
            }
         }
      }
   ...
   private void fillTableForChild1Child2(SuffixTreeNode curr1, SuffixTreeNode curr2) {
      ...
      try {
         if(prevI < this._previousResults.length && prevJ < this._previousResults[currI].length) {
            int[][] curr2children = (int[][])this._previousResults[prevI][prevJ];
            int[][] i = (int[][])this._previousResults[currI][prevJ];
            int[][] curr2child = (int[][])this._previousResults[prevI][currJ];
            if(this._monitor[prevI][prevJ]) {
               this._table = new int[this._maxLength + 2][this._maxLength + 2];
               int prevlastCol = curr2children[1].length + 1;
               int prevlastRow = curr2children[0].length + 1;
               int var30 = general.Utils.min(prevlastCol + 1 + curr2.labelEnd - curr2.labelStart, this._table[0].length - 1);
               int lastRow = general.Utils.min(prevlastRow + 1 + curr1.labelEnd - curr1.labelStart, this._table.length - 1);
               int[] prevrowheader = i[0];
               int[] prevcolheader = curr2child[1];
               int[] prevlastRowVals = curr2child[2];
               int[] prevlastColVals = i[3];
               int allLeaves1 = 2;
               ...
               List var34 = null;
               List var35 = null;
               if(this._produceOutput && var30 - 1 >= this._minLength && lastRow - 1 >= this._minLength) {
                  var34 = this.getAllLeaves(curr1);
                  var35 = this.getAllLeaves(curr2);
               }

               int tableToSave = 2;

               int rowHeader;
               for(rowHeader = 0; tableToSave <= var30; ++rowHeader) {
               ...
               int colHeader;
               for(rowHeader = prevlastRow + 1; tableToSave <= lastRow; ++rowHeader) {
                  for(colHeader = general.Utils.max(prevlastCol + 1, rowHeader - this._maxDiff); colHeader <= general.Utils.min(var30, rowHeader + this._maxDiff); ++colHeader) {
                     ...
                     if(this._produceOutput && tableToSave - 1 >= this._minLength && colHeader - 1 >= this._minLength && this._table[tableToSave][colHeader] <= this._maxDiff && this._seq1[this._table[tableToSave][0]] != this._terminator1 && this._seq2[this._table[0][colHeader]] != this._terminator2) {
                        this.addToSolution(var34, var35, tableToSave - 1, colHeader - 1);
                     }
                  }
               ...
               for(colHeader = 2; colHeader <= lastRow; ++colHeader) {
                  var37[colHeader - 2] = this._table[colHeader][0];
               }

               var36[0] = var37;
               int[] var38 = new int[var30 - 1];

               for(int lastRowVals = 2; lastRowVals <= var30; ++lastRowVals) {
                  var38[lastRowVals - 2] = this._table[0][lastRowVals];
               }

               var36[1] = var38;
               int[] var39 = new int[var30 - 1];
               int[] leftprevlastRowVals = i[2];
               boolean continuecompare = false;

               int lastColVals;
               for(lastColVals = 2; lastColVals <= prevlastCol; ++lastColVals) {
                  var39[lastColVals - 2] = leftprevlastRowVals[lastColVals - 2];
                  if(!continuecompare && this.infinity(leftprevlastRowVals[lastColVals - 2]) <= this._maxDiff) {
                     continuecompare = true;
                  }
               }

               for(lastColVals = prevlastCol + 1; lastColVals <= var30; ++lastColVals) {
                  var39[lastColVals - 2] = this._table[lastRow][lastColVals];
               ...
               var36[2] = var39;
               int[] var40 = new int[lastRow - 1];
               int[] upperprevlastColVals = curr2child[3];

               int i1;
               for(i1 = 2; i1 <= prevlastRow; ++i1) {
                  var40[i1 - 2] = upperprevlastColVals[i1 - 2];
                  if(!continuecompare && this.infinity(upperprevlastColVals[i1 - 2]) <= this._maxDiff) {
                     continuecompare = true;
                  }
               }

               for(i1 = prevlastRow + 1; i1 <= lastRow; ++i1) {
                  var40[i1 - 2] = this._table[i1][var30];
                  if(!continuecompare && this.infinity(this._table[i1][var30]) <= this._maxDiff) {
                     continuecompare = true;
                  }
               }

               var36[3] = var40;
               this._previousResults[curr1.orderNumber][curr2.orderNumber] = var36;
               this._monitor[curr1.orderNumber][curr2.orderNumber] = continuecompare;
            }
         }
      } catch (Exception var29) {
         general.Utils.printEDMatrix(this._table);
         System.out.println();
      ...
      List var31 = curr2.children;
      if(var31 != null && var31.size() > 0) {
         for(int var32 = 0; var32 < var31.size(); ++var32) {
            SuffixTreeNode var33 = (SuffixTreeNode)var31.get(var32);
            this.fillTableForChild1Child2(curr1, var33);
            if(var32 == var31.size() - 1) {
               this._previousResults[curr1.parentOrderNumber][var33.parentOrderNumber] = null;
            }
         }
      }
   ...
   private void fillTableForChild1Root2(SuffixTreeNode curr1, SuffixTreeNode root2) {
      byte prevJ = 0;
      int prevI = curr1.parentOrderNumber;
      if(prevI < this._previousResults.length && prevJ < this._previousResults[prevI].length) {
         int[][] prevValues = (int[][])this._previousResults[prevI][prevJ];
         if(this._monitor[prevI][prevJ]) {
   ...
   private void fillTableForRoot1Children2(SuffixTreeNode root1, List children2) {
      for(int c = 0; c < children2.size(); ++c) {
         ...
         byte prevI = 0;
         if(prevI < this._previousResults.length && prevJ < this._previousResults[prevI].length) {
            int[][] grandchildren = (int[][])this._previousResults[prevI][prevJ];
            if(this._monitor[0][prevJ]) {
               int[] prevrowheader = grandchildren[0];
               int[] prevcolheader = grandchildren[1];
               int[] prevlastRowVals = grandchildren[2];
               int[] prevlastColVals = grandchildren[3];
               this._table = new int[this._maxLength + 2][this._maxLength + 2];
               int prevlastCol = prevcolheader.length + 1;
               int prevlastRow = prevrowheader.length + 1;
               ...
               for(allLeaves2 = 0; allLeaves1 <= prevlastRow; ++allLeaves2) {
                  this._table[allLeaves1][0] = prevrowheader[allLeaves2];
                  ++allLeaves1;
               }
   ...
   public static void main(String[] args) {
      ...
      BYGU1 var35 = new BYGU1(tree1, tree2, seq1, terminator1, seq2, terminator2, maxDiff, minLen);
