...
public class BYGUmap {
   ...
   public BYGUmap(SuffixTreeNode tree1, SuffixTreeNode tree2, String seq1, String terminator1, String seq2, String terminator2, int maxDiff, int minLen) {
      this._tree1 = tree1;
      this._tree2 = tree2;
   ...
   private void processSubtrees(SuffixTreeNode root1, SuffixTreeNode root2) {
      if(!this.freeEnds || this._seq1[root1.labelStart] == this._seq2[root2.labelStart]) {
         ...
         this.totalchildren1 = root1.totalChildren + 1 + root2.totalChildren + 1;
         System.out.println("Now processing root1 with " + this.totalchildren1 + " children via root2 with " + root2.totalChildren + " children.");
   ...
   private void fillTableForChild1Child2(SuffixTreeNode curr1, SuffixTreeNode curr2) {
      ...
      int[][] cornerVals = (int[][])this._previousResults.get(new Long((long)(prevI * this.totalchildren1 + prevJ)));
      int[][] leftVals = (int[][])this._previousResults.get(new Long((long)(currI * this.totalchildren1 + prevJ)));
      int[][] upperVals = (int[][])this._previousResults.get(new Long((long)(prevI * this.totalchildren1 + currJ)));
      int i;
      if(this._monitor.get(new Long((long)(prevI * this.totalchildren1 + prevJ))) != null) {
         this._table = new int[this._maxLength + 2][this._maxLength + 2];
         int curr2children = cornerVals[1].length + 1;
         i = cornerVals[0].length + 1;
         if(i >= this._table.length - 2 || curr2children >= this._table[i].length - 2) {
            return;
         }
         ...
         var33[3] = var37;
         this._previousResults.put(new Long((long)(curr1.orderNumber * this.totalchildren1 + curr2.orderNumber)), var33);
         if(continuecompare) {
            this._monitor.put(new Long((long)(curr1.orderNumber * this.totalchildren1 + curr2.orderNumber)), new Boolean(continuecompare));
         }
      }
   ...
   private void fillTableForChild1Root2(SuffixTreeNode curr1, SuffixTreeNode root2) {
      byte prevJ = 0;
      int prevI = curr1.parentOrderNumber;
      int[][] prevValues = (int[][])this._previousResults.get(new Long((long)(prevI * this.totalchildren1 + prevJ)));
      if(this._monitor.get(new Long((long)(prevI * this.totalchildren1 + prevJ))) != null) {
         ...
         if(prevlastRow >= this._table.length - 2 || prevlastCol >= this._table[prevlastRow].length - 2) {
            return;
         }
         ...
         var26[3] = var30;
         this._previousResults.put(new Long((long)(curr1.orderNumber * this.totalchildren1 + 0)), var26);
         if(var28) {
            this._monitor.put(new Long((long)(curr1.orderNumber * this.totalchildren1 + 0)), new Boolean(var28));
         }
      }
   ...
   private void fillTableForRoot1Children2(SuffixTreeNode root1, List children2) {
      for(int c = 0; c < children2.size(); ++c) {
         ...
         if(this._monitor.get(new Long((long)(prevI * this.totalchildren1 + prevJ))) != null) {
            ...
            var29[3] = var33;
            this._previousResults.put(new Long((long)(0 * this.totalchildren1 + child2.orderNumber)), var29);
            if(var32) {
               this._monitor.put(new Long((long)(0 * this.totalchildren1 + child2.orderNumber)), new Boolean(var32));
            }
         }
   ...
   private void appendChildLeaves(List res, SuffixTreeNode parent) {
      if(parent.children != null && parent.children.size() != 0) {
         ...
         res.add(new Long((long)parent.leafNumber));
      }
   }
   ...
   private void fillTableForRoots(SuffixTreeNode root1, SuffixTreeNode root2) {
      ...
      var17[3] = var20;
      this._previousResults.put(new Long(0L), var17);
      if(var19) {
         this._monitor.put(new Long(0L), new Boolean(var19));
      }
   ...
   private void addToSolution(List positions1, List positions2, int len1, int len2) {
      for(int i = 0; i < positions1.size(); ++i) {
         int startI = ((Long)positions1.get(i)).intValue();
         int endI = startI + len1 - 1;

         for(int j = 0; j < positions2.size(); ++j) {
            int startJ = ((Long)positions2.get(j)).intValue();
   ...
   public static void main(String[] args) {
      ...
      BYGUmap var35 = new BYGUmap(tree1, tree2, seq1, terminator1, seq2, terminator2, maxDiff, minLen);
